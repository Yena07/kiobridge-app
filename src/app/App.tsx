  ...
  const rows: { title: string; body: string }[] = [
    ...
    },
    {
      ...
      body: guest
        ...
        : "계정 화면의 ‘이 기기에서 정보 지우기’를 누르면 이 기기에 있는 내용이 모두 사라지고, 서버에 올라간 주문표도 함께 지워져요. 다시 로그인해도 주문표는 남아 있지 않아요. 다만 키오스크에 보낸 주문 기록은 서버에 남아요 — 그건 아직 지우는 길이 없어서요.",
    },
  ];
  return rows;
};
              ...
              onClearLocal={() => set확인대기({
                ...
                run: () => {
                  ...
                  if (계정) {
                    ...
                    void Promise.all(sheets.map((p) => account.deleteSheet(userId, p.id).catch(() => {})));
                  }
                  setSheets([]); setName("");
