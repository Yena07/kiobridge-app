...
npm run build       # 빌드 (목으로 구동)
npm run build:team  # 빌드 (실제 백엔드로 구동) — 배포는 이걸 씁니다
npm test            # 테스트
npm run typecheck
```

목이냐 실서버냐를 정하는 것은 **빌드 모드 하나**입니다. `--mode team` 이면
`client.ts` 가 `createTeamBackend()` 를 쓰고, 아니면 `mock.ts` 를 씁니다.
`vercel.json` 의 `buildCommand` 가 `npm run build:team` 이라, 배포본은 실제
백엔드로 나갑니다.

### 실제 백엔드로 붙여 볼 때

킷 → 백엔드 → 프론트 순서로 띄웁니다. **킷이 먼저입니다** — 백엔드가 부팅하며 킷을 부릅니다.
셋 다 끝나지 않는 명령이라 **터미널 세 개**가 필요합니다. 경로는 이 저장소 루트 기준입니다.

```bash
# 터미널 1
cd <킷> && npm run start:api      # :4000
# 터미널 2
cd backend && ./gradlew bootRun   # :8080  (JDK 21) — 팀 백엔드 사본
# 터미널 3
